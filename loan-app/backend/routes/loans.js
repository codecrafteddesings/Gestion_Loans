...
function calculateDueDate(term, frequency) {
    ...
    if (frequency === 'daily') {
        ...
        now.setDate(now.getDate() + term * 14);
    } else if (frequency === 'monthly') {
        now.setMonth(now.getMonth() + term);
    } else if (frequency === 'yearly') {
        now.setFullYear(now.getFullYear() + term);
    }
    return now;
} 
...
router.get('/search', async (req, res) => {
    ...
    try {
        const loans = await Loan.find({
            $or: [
                { 'clientId': mongoose.Types.ObjectId.isValid(query) ? query : null },
                { 'clientId.name': { $regex: query, $options: 'i' } },
                { status: { $regex: query, $options: 'i' } }
            ]
        }).populate('clientId', 'name email');
        res.json(loans);
